...
(async () => {
    ...
    program.name('youtube-dl-exec-script')
        .version('1.4.0') // Version is hardcoded here because on non-Windows systems this script has to be called by node instead of npm
    ...
    video = new Video({
        ...
        resolution: infojsonData.resolution
            ...
            || undefined,
        tbr: ffprobeData?.format?.bit_rate || infojsonData.tbr,
        abr: ffprobeAudioStream?.bit_rate || infojsonData.abr,
        acodec: ffprobeAudioStream?.codec_name || infojsonData.acodec,
        asr: ffprobeAudioStream?.sample_rate || infojsonData.asr,
        vbr: ffprobeVideoStream?.bit_rate || infojsonData.vbr,
        fps: ffprobeVideoStreamFps || infojsonData.fps,
        vcodec: ffprobeVideoStream?.codec_name || infojsonData.vcodec,
        url: infojsonData.url,
        ext: ext || infojsonData.ext,
